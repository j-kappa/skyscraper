...
- **Adaptive background** — the 3D scene background matches the page's actual background color
- **Shadow mapping** — fixed low-angle directional light casts PCF soft shadows across the cityscape
- **Width-aware dampening** — full-width sections flatten into ground planes while smaller elements (cards, buttons, images) rise as buildings
- **Media priority** — images and media always render above overlapping container divs
- **Rounded corners** — elements with `border-radius` render as extruded rounded shapes
- **CORS proxy with fallback** — external stylesheets and images are fetched through multiple proxies and inlined so html2canvas can capture them
...
src/
  ...
  sun.ts        — fixed low-angle directional light with shadow mapping
...
1. **Fetch** — the target page's HTML is retrieved through a CORS proxy with fallback (`fetcher.ts`)
...
5. **Build** — each block becomes a Three.js box (or rounded extrusion) whose height is normalized relative to the page's max DOM depth, with the screenshot region mapped onto its top face (`builder.ts`)
6. **Light** — a fixed low-angle directional light casts long shadows across the scene (`sun.ts`)
